...
```
The above script performs what exactly has been explained: the result is a much **more contained** $19$ files with unique stack traces: this process is **correct** as every possible crash is still contained in above batch. Notice that ASLR had to be temporarily disabled during its execution, otherwise every stack trace would be **unique** due to address randomization.

### AddressSanitizer: Requested allocation size exceeds maximum
<!-- id:0 --> 
This error is thrown by `Acquire_Magick_Memory()` in the following line 
```c
#if !defined(MAGICKCORE_ZERO_CONFIGURATION_SUPPORT)
  memory=memory_methods.acquire_memory_handler(size == 0 ? 1UL : size);
#else
```
The `acquire_memory_handler` calls `malloc()` trying to allocate a **very large** amount of memory (`size = 0x71446e912aaaa99`) higher than the maximum allowed of `0x10000000000`. 
```c
size=count*quantum;
```
This happens because the program is trying to allocate heap space through `SetQuantumDepth()` to place the image in, this is done by reading information about the file's "Quantum", a structure used to represent pixels in ImageMagick 
```c
...
```
The overlap then happens due to **malformations** in the fuzzed file.
